...
- `번호`, `인증일자`, `프로젝트번호`, `회사명`, `제품명`, `시험PL`은 기준정보로 취급해 자동 갱신하지 않는다.
- 자동 점검 결과는 `점검결과`부터 `홍보이미지`까지의 점검 컬럼만 갱신한다.
- `점검결과` 값은 `완료`, `수정 필요`, `보류`를 사용한다.
- 산출물별 점검 컬럼은 현재 dry-run에서 `정상`, `부적합`, `X` 샘플 값을 사용한다.
- 실제 규칙 구현 단계에서 산출물별 컬럼과 규칙 결과의 매핑을 확정한다.
