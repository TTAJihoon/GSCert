...
- 패키징된 Windows 앱으로 실제 ECM 제출물 폴더 점검 후 웹 결과와 비교
- 실제 정상 산출물 기준 1~18번 전체 규칙 PASS 여부 검증
- 실제 규칙 결과와 `ecmlist.db` 산출물 컬럼 간 매핑 검증
- 테스트가 끝난 뒤 download-review 시작 가능 시간을 운영 기준 `20:00-07:00`으로 복구
...
- 기준 테이블은 `ecm_list`다.
- `번호`부터 `WD`까지는 기준정보로 보고 수정하지 않는다.
...
- `sync_sheets.py`는 Google Sheet F열을 `WD` 컬럼에 저장한다.
- `main/data/ecmlist.db`, `main/data/ecmlist2.db`에는 `WD` 컬럼을 추가했다.
...
- 운영 Google Sheet에서 F열의 WD 값이 모든 대상 프로젝트에 안정적으로 채워져 있는지 검증
...
1. 패키징된 `GSCertLocalReview.exe`로 실제 ECM 제출물 폴더를 선택해 웹 점검 결과와 비교한다.
2. 실제 정상 산출물 zip/폴더로 1~18번 전체 규칙 PASS 여부를 검증한다.
3. 5~9단계 통합 검증: 실제 ECM 서버에서 전체 다운로드 파이프라인을 테스트한다.
4. 실제 다운로드 산출물 기준으로 전체 점검 규칙과 `ecmlist.db` 점검 컬럼 매핑을 검증한다.
5. 테스트가 끝나면 download-review 시작 가능 시간을 `20:00-07:00`으로 복구한다.
