...
- 1~13번, 15~18번 실제 점검규칙
- 14번 시험기록서는 아직 구현 전이다.
- 구현된 주요 규칙 타입은 `required_artifact_file`, `document_artifact_check`, `excel_feature_list_check`, `test_plan_document_check`, `image_screenshot_folder_date_check`, `test_case_check`, `defect_report_check`, `inspection_checklist_check`, `rawdata_folder_structure_check`, `test_report_document_check`, `quality_inspection_table_check`, `quality_evaluation_report_check`이다.
...
- 14번 시험기록서를 수동 다운로드형 PDF 산출물로 제공할지 최종 확인
