...
- 테스팅 에이전트의 시험 이력 조회, 유사 시험 조회, 보안성 결함리포트 생성 화면은 좌측 검색 사이드바에 내용이 넘칠 때 가로/세로 스크롤바를 표시한다. 시험 이력 조회의 날짜 조건은 `시험일자`/`인증일자` 토글과 하나의 기간 입력으로 통합했으며 선택한 기준에 맞춰 검색한다.
- 시험 이력 조회의 시험일자 검색은 정상 날짜로 매칭된 결과의 최소·최대 `serial_number` 구간 안에 있는 공백/비정상 시험기간 행도 함께 표시한다. 구간 밖의 비정상 행과 기간 밖의 정상 행은 제외하며, 최종 결과는 검색 기준과 관계없이 `serial_number` 내림차순으로 정렬한다.
