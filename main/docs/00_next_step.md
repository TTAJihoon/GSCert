...
1. 실제 샘플 ZIP 또는 운영 ECM 프로젝트로 파일명 개정 버전 dedup, 9-6 버전 추출, 10번 결함리포트 시트명 공백 무시, `수정 내용` 팝업, 모달 전체 폴더 다운로드를 브라우저에서 확인한다.
2. UI 변경 후 서버를 재시작하고 `/download-review/`에서 정적 파일 `?v=24`가 로드되는지 확인한다.
