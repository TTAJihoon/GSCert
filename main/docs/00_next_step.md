...
- `/similar/`의 수동 입력은 LLM 추천 5개와 원본 문장 1개를 페이지 내 `검색 2단계`에 표시하며 원본 문장만 기본 선택한다.
- 자동 입력은 업로드 문서 기반 추천 4개와 원본 추출 요약 1개를 표시하며 첫 추천 문장만 기본 선택한다. PDF/DOCX/PPTX/XLSX/TXT를 파싱한다.
- 여러 문장을 선택하면 문장별 FAISS 및 LLM 유사도를 제품별로 평균 내어 종합 순위를 표시한다.
- 인증일자 검색 기간은 시작일 기본값 `2017-01-01`, 종료일 미지정 시 현재까지이며 FAISS 후보 선정 단계부터 적용한다.
- 결과 카드는 제품 개요를 주 정보로 강조하고 업체명-제품명을 보조 정보로 표시한다. WD 값은 진한 글자색으로 표시하며 날짜순/유사도순 버튼은 활성 정렬 방향을 함께 보여준다.
- 첫 결과 카드의 복사 안내문은 결과 스크롤 영역에 잘리지 않도록 전역 고정 레이어로 표시한다.
- 관련 단위 테스트 7건, Django 검사, JavaScript/Python 문법 검사, 실제 LLM 추천 인라인 단계와 브라우저 표시를 확인했다.
- 현재 개발 작업공간에는 `main/data/faiss_bge_m3_ko.idmap.index`가 없어 실제 제품 데이터 검색은 실행하지 못했다. 인덱스가 있는 운영 환경에서 다중 문장 실검색을 최종 확인한다.
...
4. FAISS 인덱스가 있는 환경에서 `/similar/`의 복수 문장 선택 후 평균 유사도 순위를 실데이터로 확인한다.
