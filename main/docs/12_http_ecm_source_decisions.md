...
**성능(2026-07-07 추가):** 같은 프로젝트 변형 폴더(신청/계약/완료…)는 목록에서 연속으로 붙어 있으므로,
매칭 블록이 시작된 뒤 다른 프로젝트 폴더가 나오면 그 폴더 스캔을 조기 종료한다(`find_project_folder`).
폴더 목록은 `getChildren` 한 번으로 받으므로 프로젝트 수(N)에 비례하는 요청은 없고, 조기 종료는
대량 폴더에서 뒤쪽 항목의 불필요한 메모리 매칭만 줄인다.
...
- 전송 중 연결이 끊기는 경우(`IncompleteRead` 등 `download_bytes` 자체가 던지는 예외)도 크기 불일치와 동일하게 취급한다.
- 검증/다운로드 실패 시 1회 재다운로드까지 시도한다. 그래도 실패하면(실제 사례: ECM에 저장된 파일 자체가 손상되어 항상 같은 지점에서 끊기는 경우) **그 파일 하나만 누락시키고 프로젝트 fetch는 성공으로 계속 진행**한다(`FetchResult.failed_files`에 `[폴더] 파일명: 사유` 형태로 누적, `download_partial_failure` 경고 로그로 남김). 이전에는 파일 하나의 영구 실패가 프로젝트 전체를 실패시켰으나, 이제는 누락된 파일이 필요한 규칙만 "파일을 찾지 못함"으로 자연스럽게 부적합 처리되고 나머지 규칙은 정상 점검된다.
