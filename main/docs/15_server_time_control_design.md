...
`/download-review/`에서 194 서버의 날짜와 시간을 임시로 과거 시각으로 변경하고, 5분 뒤 정상 시각으로 복구한다.
...
6. 최초 변경으로부터 단조 증가 시계 기준 5분 뒤 정상 시각으로 자동 복구한다.
7. 변경한 사용자는 같은 이름과 PIN을 입력해 조기 복구하거나 다른 과거 시각으로 재설정할 수 있다.
8. 재설정에 성공하면 그 시점부터 5분을 다시 계산한다.
9. 이름/PIN이 다르거나 revision이 오래된 요청은 거부한다.
10. 잠금은 5분 경과가 아니라 정상 시각 복구와 검증이 성공한 뒤 해제한다.
...
1. 과거 시각 설정 후 W32Time이 5분 전에 임의 복구하지 않는지.
...
- `run_server_time_agent`: W32Time 중지, Windows 시스템 시각 변경, 단조 5분 타이머, NTP 복구·검증.
- `GSCertTimeControl` Windows 서비스와 설치 스크립트: 자동 시작과 실패 시 재시작.
- `/download-review/`의 서버 시간 설정 팝업: 이름, 4자리 PIN, 과거 분 단위 시각, 설정자·남은 시간 표시.
- 기존 download-review 시간 제한은 제거했고 새 작업은 즉시 queued 처리한다.
