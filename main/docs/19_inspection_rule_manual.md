...
- 구현 메모: 파일/내용 검사와 PDF 캡처 저장/UI 조회 버튼이 구현되어 있다.
...
| `test_case_check` | 테스트케이스 Excel 내부값과 잔여 결함 F 개수 검사 |
| `defect_report_check` | 결함리포트 버전/시트/보고일자 검사와 `{잔여결함수}`, `{H}`, `{R}` 산출 |
| `inspection_checklist_check` | 점검표 Excel/PDF 검사와 `{측정항목별점수표}` 산출 |
