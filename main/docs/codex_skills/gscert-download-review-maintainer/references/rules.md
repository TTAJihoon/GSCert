...
Manual pass overrides live in shared PostgreSQL `reference` through `DownloadReviewManualOverride` (`inspection_manual_override`). They are keyed by `center_code + project_number + rule_code + sub_check_key`, not by `inspection_result.id`, because reinspection can replace result rows. `sub_check_key=""` is the legacy rule-level override; non-empty keys such as `sub-1` apply only to the matching detail row.
...
- `{버전}`: version parsed from product name. Prefer `v`/`ver`/`version` suffixes such as `v1`, `v1.0`, or `version Enterprise`; otherwise use the final whitespace-delimited token. If that final token combines text and a numeric suffix, keep the text in `{제품명}` and use only the numeric suffix as `{버전}` (for example `EBS ISM3.0` -> product `EBS ISM`, version `3.0`).
- `{pl}`, `{PL}`: 시험PL
- `{wd}`, `{WD}`: WD
...
## Artifact Revision Dedup

- Before artifact rules inspect contents, ECM file candidates are deduplicated by filename-end revision tokens such as `v1`, `v1.0`, `v1.1`, `ver 1.1`, `version 1.1`, and `1.1`.
- For the same artifact identity, versioned files supersede unversioned files.
- Keep the latest minor per major. If the same major/minor appears more than once, keep the file with the newest modified time.
- If an old version is in the configured folder and a newer version of the same artifact is uploaded elsewhere, the newer file is pulled into the candidate set and the old version is discarded.
- Rule 10 결함리포트 keeps one latest-minor file for each major round. Example: `v1.0`, `v1.1`, and `v2.0` become `v1.1` and `v2.0`.
