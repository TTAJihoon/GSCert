...
def _defect_report_xlsx_split_title(project_number, sheets, *, header=None, footer=None, summary_cover_title=None):
    rows_by_sheet = []
    for sheet_name in sheets:
        if sheet_name in ("최종결함리포트", "시험분석자료"):
        ...
        cover_title = sheet_name
        if sheet_name == "시험분석자료" and summary_cover_title:
            cover_title = summary_cover_title
        rows = [
...
class DownloadReviewJobsApiTests(TestCase):
    ...
    def test_promotional_material_passes_with_hwpx_file_without_forbidden_word(self):
        # 한글 신형식(hwpx)도 인정한다.
        self._create_promotional_material_rule()
        project_dir = Path(self.temp_dir.name) / "promo_hwpx_ok"
        (project_dir / "홍보").mkdir(parents=True)
        (project_dir / "홍보" / "홍보자료.hwpx").write_bytes(b"hwpx")
