...
def fetch_texts_from_pg():
    from main.models import SwData

    qs = (
        SwData.objects.using("reference")
        .filter(start_date__gte="2016-01-01")
        .exclude(product_desc="")
        .values_list("serial_number", "product_desc")
        .order_by("serial_number")
    )
    ids = [row[0] for row in qs]
    texts = [row[1] for row in qs]
    return ids, texts
