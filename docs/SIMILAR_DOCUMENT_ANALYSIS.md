...
7. 최종 LLM 호출로 원본 추출 요약 1개, 의미 보존형 추천 요약 4개,
   문서에서 확인된 주요 기능 목록을 생성한다.
8. 사용자는 기본 요약 후보 아래에서 최대 100자의 검색 문장을 직접 추가할 수 있다.
9. UI는 파일별 성공·실패·경고, 실제 반영 블록 수와 주요 기능을 표시한다.
