...
규칙 결과 상세 팝업은 아래 버튼에서 연다.

- 프로젝트 선택 탭: 프로젝트 행의 `상세` 버튼
  - 최신 완료/수정 필요/보류 결과를 조회한다.
  - 완료 또는 수정 필요이면 규칙별 결과 표를 표시한다.
  - 보류 또는 실패이면 사용자용 오류 메시지를 표시한다.

- 작업 조회 탭: 선택한 작업의 프로젝트 행 `상세` 버튼
  - 해당 작업 요청 기준의 규칙 결과를 조회한다.
  - 같은 프로젝트를 여러 번 실행한 경우에도 선택한 작업의 결과만 보여준다.

- 현재 작업 진행 상황 탭: 실패/보류 행의 오류 상세 버튼
  - 진행 중 화면에서는 규칙 결과보다 실행 실패 원인 확인을 우선한다.
...
서버 내부 절대경로와 stack trace는 사용자 화면에 표시하지 않고 관리자 로그에만 저장한다.
