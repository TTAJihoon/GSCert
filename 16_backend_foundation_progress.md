...
### workflow.db 작업/락 저장소 계층

`playwright_job/workflow_repository.py`를 추가했다.

역할:

- `main/data/workflow.db`에 작업 실행 이력을 저장한다.
- 사용자 요청 1건을 `automation_job`으로 저장한다.
- 작업에 포함된 프로젝트별 처리 상태를 `automation_job_project`로 저장한다.
- 동시에 하나의 작업만 실행되도록 `automation_lock` 전역 락을 관리한다.
- 작업자 프로세스 생존 확인을 위해 `heartbeat_at` 값을 갱신할 수 있다.
...
- 프로젝트별 상태 변경
- 완료/실패 프로젝트 수 갱신
- 전역 락 획득, 해제, heartbeat 갱신

락 기준:

- 락 row는 `automation_lock.id = 'global'` 한 개만 사용한다.
- 이미 `locked = 'Y'`이면 새 작업은 락을 얻지 못한다.
- 락을 잡은 `job_id`와 같은 작업만 락을 해제할 수 있다.

## 다음 작업 후보

UI 파일을 건드리지 않고 이어갈 수 있는 다음 작업은 아래 순서가 적합하다.

1. 작업 생성/진행상태/결과 조회용 서비스 계층 구현
2. 별도 작업자 프로세스의 dry-run 골격 구현
3. 다운로드 폴더 경로 설정값 관리 구현
4. 실제 `reference.db` 샘플 연결 검증
5. 작업 락 heartbeat 만료/복구 정책 확정
