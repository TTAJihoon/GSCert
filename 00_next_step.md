...
- `진행상황` 탭 이름을 `프로젝트별 진행 상황`으로, `결과 조회` 탭 이름을 `작업 조회`로 바꿨다.
- 프로젝트별 진행 상황 탭의 `작업 없음 상태 보기` 버튼은 active 작업이 있어도 빈 작업 화면을 강제로 미리 볼 수 있게 했다.
...
- 프로젝트별 진행 상황 탭에서 active 작업이 있는 동안 빈 작업 패널이 보이지 않음 확인
