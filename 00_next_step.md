...
`codex-job-runner-persistence` 브랜치는 현재 UI 목업, 프로젝트 목록 API, 작업 요청/대기/예약 API, dry-run worker, `ecmlist.db` write-back, 작업 조회 API까지 반영되어 있다.
...
프로젝트 선택/현재 작업 진행 상황/작업 조회 UI를 보강했다.
...
- 현재 작업 진행 상황 탭에서 active 작업이 있는 경우 `진행 중인 작업이 없습니다.` 패널이 함께 보이지 않도록 고쳤다.
- active 작업을 보여주는 탭 명칭을 `현재 작업 진행 상황`으로, 작업 이력 탭 명칭을 `작업 조회`로 정리했다.
- 현재 작업 진행 상황 탭의 `작업 없음 상태 보기` 버튼은 active 작업이 있어도 빈 작업 화면을 강제로 미리 볼 수 있게 했다.
- 완료 프로젝트의 작업상태는 체크박스 비활성 상태와 맞게 `완료`로 표시한다.
- 프로젝트 선택 탭의 상세 버튼은 최근 완료/실패 작업의 점검규칙 결과를 바로 조회한다.
- `GET /api/projects/<프로젝트번호>/latest-results/` API를 추가했다.
- active 작업 재조회 후 프로젝트 선택 요약의 예약 안내 문구도 함께 갱신되게 했다.
- 작업 조회 탭의 예약됨/대기중 작업 카드에 `예약 취소` 버튼을 추가했다.
- `POST /api/jobs/<job_id>/cancel/` API를 추가했고, 예약됨/대기중 작업만 취소할 수 있다.
- 취소된 작업의 프로젝트는 `건너뜀` 상태로 바뀌며, 같은 프로젝트는 다시 요청할 수 있다.
- 예약 취소 UI는 취소 API 성공 응답을 받으면 완료 모달을 먼저 표시하고, 프로젝트/작업 목록 재조회는 뒤에서 정리한다.
...
- 로컬 `workflow.db` 상태는 UI 취소/worker 검증을 반복하면서 바뀔 수 있다.

주의:

- `대기중` 작업은 작업 시작 가능 시간에는 걸리지 않은 상태지만, worker가 실행되어야 실제 처리된다.
- 전체 시간 설정(`0-24`)에서도 작업이 계속 `대기중`이면 `run_download_worker` 실행 여부, `DownloadReviewLock` 잔류 여부, worker 로그를 먼저 확인한다.
- 로컬 UI 검증에서는 아래 명령으로 대기 작업 1건을 dry-run 처리할 수 있다.
...
- 현재 작업 진행 상황 탭에서 active 작업이 있는 동안 빈 작업 패널이 보이지 않음 확인
- `작업 없음 상태 보기` 버튼으로 빈 작업 화면 미리보기 확인
- 작업 조회 탭에서 기존 예약 작업 취소 플로우 확인

추가 확인:

- `TTA-26-00200` 예약 취소 UI에서 `작업 취소 완료` 모달 표시 확인
- `TTA-26-00200` 작업을 다시 생성한 뒤 dry-run worker 1회 실행 확인
- 생성 작업 `665305e3-a067-4913-ba7d-6c482371a140`은 dry-run worker에서 `완료` 처리됨
- 따라서 전체 시간 설정 상태에서 `대기중`으로 남아 있으면 시간창보다 worker 미실행 상태를 먼저 확인해야 한다.
- 로컬 `ecmlist.db`의 `TTA-26-00200`은 dry-run 확인 과정에서 `점검결과=완료`로 갱신되었다.

## 2026-05-13 현재 작업 진행/상세 결과 보강

- 완료 프로젝트는 프로젝트 선택 탭의 작업상태도 `완료`로 표시한다.
- 프로젝트 선택 탭의 `상세` 버튼은 최근 완료/실패 작업의 점검규칙 결과를 직접 조회한다.
- `GET /api/projects/<프로젝트번호>/latest-results/` API를 추가했다.
- active 작업 하나만 보여주는 화면의 명칭을 `현재 작업 진행 상황`으로 정리했다.
- 로컬에서 대기중이던 작업 2건은 dry-run worker 실행 후 완료 처리됨을 확인했다.
- `start_worker.ps1` 실행 시 Windows 환경에 `Path`/`PATH`가 동시에 잡혀 `Start-Process`가 실패하는 경우가 있어, worker 시작 전에 프로세스 환경변수를 정규화하도록 수정했다.
